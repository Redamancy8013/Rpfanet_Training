...
from ..astyx.object3d_astyx import Object3dAstyx, inv_trans
...
class VodDataset(DatasetTemplate):
    ...
    def get_label(self, idx):
        ...
        with open(label_file, 'r') as f:
            ...
            for line in data:
                ...
                obj['dimension3d'] = np.array((float(label[8]), float(label[9]), float(label[10])), dtype=np.float32)
                obj['score'] = label[15]
                obj['classname'] = label[0]
                obj['occlusion'] = label[2]
                obj['center3d'] = np.array((float(label[11]), float(label[12]), float(label[13])), dtype=np.float32)

                angle = float(label[14]) + np.pi / 2
                ...
                # 重新排列为 (w, x, y, z) 格式
                obj['orientation_quat'] = np.roll(quaternion, shift=1)
                
                objects.append(Object3dAstyx.from_label(obj))
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        # for i, item in enumerate(sample_id_list):
        #     if not hasattr(item, '__iter__'):
        #         print(f'Item {i} is not iterable')
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_vod_infos':
        ...
        create_vod_infos(
            ...
            data_path=ROOT_DIR / 'data' / 'vod',
            save_path=ROOT_DIR / 'data' / 'vod'
        )
