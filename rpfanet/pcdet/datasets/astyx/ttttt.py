...
class AstyxDataset(DatasetTemplate):
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        infos, pts = process_single_scene('000211')
...
def create_astyx_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    astyx_infos_train, pts = dataset.get_infos(num_workers=workers, has_label=True, count_inside_pts=True)
    print(f'astyx_infos_train: {astyx_infos_train}')
    ...
    # 将边界框添加到场景中
    for bbox in astyx_infos_train['annos']['gt_boxes']:
        center = bbox[0:3]
        lhw = bbox[3:6]
        rotation_quaternion = bbox[6]  # 从第7个元素开始为四元数

        R = np.array([[np.cos(rotation_quaternion), -np.sin(rotation_quaternion), 0],
                  [np.sin(rotation_quaternion), np.cos(rotation_quaternion), 0],
                  [0, 0, 1]])

        # 创建边界框
        box = o3d.geometry.OrientedBoundingBox(center=center, extent=lhw, R=R)
        # box.paint_uniform_color([0, 1, 0])  # 设置边界框颜色为绿色
        scene.add_geometry(box)

    # 添加坐标轴
    coord_axis = o3d.geometry.TriangleMesh.create_coordinate_frame(size=1.0, origin=[0, 0, 0])
    scene.add_geometry(coord_axis)

    # 设置渲染参数
    render_options = scene.get_render_option()
    render_options.point_size = 2
    # render_options.background_color = np.asarray([0, 0, 0])  # 设置背景颜色为黑色
    # render_options.point_color_option = o3d.visualization.PointColorOption.ColorOn  # 设置点云颜色为白色
